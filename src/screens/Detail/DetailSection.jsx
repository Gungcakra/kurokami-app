...
export const DetailSection = ({ item, insets, navigation }) => {
  ...
  return (
    <View>
      ...
      <View className="relative h-[550px] w-full bg-zinc-900">
        ...
        <View className="absolute inset-x-0 bottom-0 h-40 bg-gradient-to-t from-zinc-bg to-transparent" />
        ...
        {/* TOP BAR ACTIONS */}
        <View style={{ marginTop: insets.top + 10 }} className="absolute top-0 left-0 right-0 px-6 flex-row justify-between z-20">
          <TouchableOpacity
      ...
      <Animatable.View
        animation="fadeInUp"
        duration={600}
        className="px-6 -mt-20 bg-zinc-bg rounded-t-[45px] pt-10 pb-10"
      >
          ...
          <View className="flex-row mt-6 gap-6 bg-zinc-soft/50 p-4 rounded-3xl border border-zinc-border/50">
            ...
            <View className="items-center">
              <Text className="text-white font-black text-lg">
                {item?.view_count ? (item.view_count / 1000000).toFixed(1) : '0'}M
              </Text>
          ...
          <View>
            <View className="flex-row flex-wrap justify-center gap-3 mt-6">
              {item?.taxonomy?.Artist?.map((artist) => (
                <View key={artist.taxonomy_id} className="flex-row items-center bg-zinc-soft/50 px-3 py-1.5 rounded-lg border border-zinc-border/50">
                  <Ionicons name="brush-outline" size={12} color="#A1A1AA" style={{ marginRight: 6 }} />
                  <Text className="text-zinc-300 text-[10px] font-bold uppercase">{artist.name}</Text>
                </View>
              ))}
              {item?.taxonomy?.Author?.map((author) => (
                <View key={author.taxonomy_id} className="flex-row items-center bg-zinc-soft/50 px-3 py-1.5 rounded-lg border border-zinc-border/50">
                  <Ionicons name="person-outline" size={12} color="#A1A1AA" style={{ marginRight: 6 }} />
                  <Text className="text-zinc-300 text-[10px] font-bold uppercase">{author.name}</Text>
                </View>
              ))}
              {item?.taxonomy?.Format?.map((format) => (
                <View key={format.taxonomy_id} className="flex-row items-center bg-zinc-soft/50 px-3 py-1.5 rounded-lg border border-zinc-border/50">
                  <Ionicons name="document-text-outline" size={12} color="#A1A1AA" style={{ marginRight: 6 }} />
                  <Text className="text-zinc-300 text-[10px] font-bold uppercase">{format.name}</Text>
                </View>
              ))}
            </View>
          </View>
        ...
        <View className="mb-6">
          ...
          <Text className="text-zinc-muted leading-6 text-sm text-justify">
            {item?.description}
          </Text>
        </View>
      </Animatable.View>
