...
export const apiService = {
  getNewUpdate: (page = 1, pageSize = 30, format = "all") =>
    fetchData(
      ...
      }&type=project&page=${page}&page_size=${pageSize}&is_update=true&sort=latest&sort_order=desc`
    ),
  ...
  getManhwa: (page = 1, pageSize = 10) =>
    fetchData(
      `/v1/manga/list${
        format === "all" ? "" : "?format=manhwa"
      }&page=${page}&page_size=${pageSize}&is_recommended=true&sort=latest&sort_order=desc`
    ),

  getManga: (page = 1, pageSize = 10) =>
    fetchData(
      `/v1/manga/list${
        format === "all" ? "" : "?format=manga"
      }&page=${page}&page_size=${pageSize}&is_recommended=true&sort=latest&sort_order=desc`
    ),
  ...
  getChapterList: (
    ...
    fetchData(
      `/v1/chapter/${manhwaId}/list?page=${page}&page_size=${pageSize}&sort_by=chapter_number&sort_order=${order}${
  ...
  searchManga: (keyword, page = 1, pageSize = 15) =>
  ...
  getExplore: ({
    ...
    format = "",
    sortOrder = "desc",
    keyword = "", // Ubah 'query' menjadi 'keyword' agar sesuai dengan State Hook
  }) => {
    const genreParams =
      genres.length > 0
    ...
    if (format && format !== "all" && format !== "") url += `&format=${format}`;
    if (sort) url += `&sort=${sort}`;
    if (sortOrder) url += `&sort_order=${sortOrder}`;

    // Gunakan keyword di sini
